...
| 来源 | 内容 |
|------|------|
| `spec/requirement.md` | 「项目文档约束」章节（项目自述架构、关键约定） |
| `design.md` | 「项目文档约束」和「现有项目约定」章节 |
| code-explorer 输出 | 「项目文档摘要」（如存在） |

**重点关注**：
- 项目的目录结构约定（新文件应放在哪个目录）
- 分层规则（哪些任务涉及哪些架构层）
- 命名约定（文件、函数、变量的命名规则）
- 编码规范（提交策略、测试要求、代码风格）

**任务分解时应遵循这些约定**，确保交付物与项目现有结构一致：
- 新建文件的目录归属必须符合项目约定
- 新增组件的分层归属（domain/application/infrastructure 等）必须与项目架构一致
- 命名规范（PascalCase/camelCase/kebab-case 等）遵循项目已有约定

### 步骤1.5: 全栈模式依赖链校验 ⭐🆕

**读取 spec/requirement.md 中的全栈模式标签和数据库/接口契约需求标签**：
...
<HARD-GATE>
当 开发模式=fullstack && 需要接口契约=是 时，必须验证以下依赖链完整。
当 需要数据库设计=是 时，数据库设计必须在接口契约之前完成。
</HARD-GATE>

**依赖链验证清单**：
- [ ] （如需要数据库设计）design.md 中数据库设计章节已完成且用户已确认
- [ ] design.md 中接口契约章节已完成且前后端双方已确认
- [ ] 接口字段与数据库设计保持一致
- [ ] 前端任务依赖的接口已在接口契约中定义
- [ ] 后端任务实现的接口与接口契约一致

如果以上任何一项不满足，**暂停任务拆解**，回到 code-designer 阶段补充设计。

### 步骤1.6: 多项目任务分组 ⭐🆕

**读取 spec/requirement.md 中的「多项目协作」章节**：
- 协作模式：非 single → 执行多项目任务分组
- 协作模式：single → 按现有单项目模式执行

<HARD-GATE>
当涉及多项目时，必须读取 collaboration-plan.md 并验证：
1. collaboration-plan.md 存在
2. 所有涉及项目都有对应的任务分组
3. 跨项目依赖链在任务依赖关系中完整体现
4. 每个项目都有明确的出口门控
任一验证失败则暂停任务拆解，回到 code-designer 阶段补充协作计划。
</HARD-GATE>

**多项目任务分组规则**：
1. 从 collaboration-plan.md 读取项目依赖图和执行顺序
2. 将任务按所属项目分组
3. 在每组内应用现有的并行执行计划
4. 在项目间建立执行顺序（拓扑排序）
5. 标记跨项目依赖门控（gate）
...
```
Phase 1: project-a
  Batch 1: [T-A1, T-A2] → 并行
    ↓ (gate: project-a 所有任务完成)
Phase 2: project-b
  Batch 1: [T-B1, T-B2] → 并行
  Batch 2: [T-B3] → 执行
    ↓ (gate: project-b 所有任务完成)
Phase 3: project-c + project-d (可并行)
  project-c Batch 1: [T-C1] → 执行
  project-d Batch 1: [T-D1, T-D2] → 并行
```
...
4. **按架构层** ⭐🆕 - 数据层/领域层/应用层/表现层，下层 Task 优先于上层 Task
5. **🆕 全栈模式强制顺序**：数据库任务 → 接口契约任务 → 后端开发任务 + 前端开发任务（使用 Mock）→ 联调任务
...
- **🆕 数据库迁移** ⭐：迁移脚本执行验证
- **🆕 接口契约** ⭐：API 端点定义、Mock 数据生成（全栈模式下数据库完成后执行）
- **🆕 跨项目接口** ⭐🆕：提供方项目接口实现、消费方项目接口集成
...
- **上线脚本** ⭐：数据库迁移脚本、配置变更脚本
- **🆕 联调** ⭐（全栈模式）：前后端联调验证，替换 Mock 为真实 API
- **🆕 跨项目联调** ⭐🆕（多项目模式）：项目间接口联调验证
