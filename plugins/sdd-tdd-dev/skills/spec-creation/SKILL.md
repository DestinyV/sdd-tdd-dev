...
| 并行路 | 任务 | subagent 类型 | 搜索范围 |
|--------|------|-------------|---------|
| 路1 | 文档探索 | code-explorer agent（阶段0） | 根目录文档（CLAUDE.md、README.md 等） |
| 路2 | 历史需求探索 | general-purpose subagent | spec-dev/ 目录 |
| 路3 | 代码模式探索 | code-explorer agent（阶段2-4） | src/ 或等价源码目录 |

**路1：文档探索**：
- 分配 **code-explorer agent**，执行「阶段0：项目文档发现」
- 文档探索内部再拆分为两路并行：
  - **核心文档扫描**：CLAUDE.md / AGENTS.md / GEMINI.md / README.md
  - **扩展文档扫描**：ARCHITECTURE.md / docs/**/*.md / wiki/**/*.md / .cursor/rules/**
- 提取项目声明的架构模式、模块划分、编码规范、技术栈
- 这些文档约定是后续需求分析的最高优先级约束

**路2：历史需求探索**：
- 分配 **general-purpose subagent**
- 扫描 `spec-dev/` 目录，读取最近 3-5 个已完成需求的 requirement.md 和 design.md
- 提取需求类型分类、设计决策模式、实际交付文件清单、遇到的问题

**路3：代码模式探索**：
- 分配 **code-explorer agent**，执行阶段2-4（代码模式提取 + 路径映射 + 标准校验）
- 识别项目中已有的相似功能，确认技术实现路径
- 提取架构分层、命名约定、数据流约定

**整合流程**：
1. 同时发射三路 subagent（使用 Agent 工具 + `run_in_background: true`）
2. 等待三路全部完成
3. 按「文档摘要 → 历史需求 → 代码模式」顺序整合为「项目理解报告」
4. 删除重复内容，保留最精准的引用

**容错机制**：
- 任何一路失败（超时/错误），不阻塞其他路，标注"⚠️ 该路探索失败"
- 若文档探索全部未找到文件，标注"未发现项目自述文档，仅基于代码分析推断"
- 若历史需求目录为空，标注"无历史需求可参考"
- 至少一路成功即可进入后续场景拆解

**小型项目策略**：
- 文件数 < 200 的项目保持串行：文档探索 → 历史需求 → 代码模式
- 无需并行发射 subagent
...
7. **多项目一致性** ⭐🆕（多项目场景）：
   ...
   - 项目依赖链是否完整无遗漏
8. **项目约定对齐** ⭐🆕：spec 中的技术选型和架构假设是否与项目文档（CLAUDE.md、ARCHITECTURE.md 等）中声明的约定一致
